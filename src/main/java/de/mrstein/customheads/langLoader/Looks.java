...
public class Looks {
    ...
    public Looks(String language) {
        ...
        try {
            ...
            items.put("your_head", CustomHeads.getTagEditor().setTags(new ItemEditor(Material.SKULL_ITEM).setDamage((short) 3).setDisplayName(CustomHeads.getLanguageManager().ITEMS_YOUR_HEAD).getItem(), "chItem", "your_head", "clonable", "tempTags"));
            items.put("help", CustomHeads.getTagEditor().setTags(new CustomHead(getURLFrom("eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvNmE1M2JkZDE1NDU1MzFjOWViYjljNmY4OTViYzU3NjAxMmY2MTgyMGU2ZjQ4OTg4NTk4OGE3ZTg3MDlhM2Y0OCJ9fX0=")).setDisplayName(CustomHeads.getLanguageManager().ITEMS_HELP).setLore(CustomHeads.getLanguageManager().ITEMS_HELPLORE).toItem(), "chItem", "help", "needsPermission", "heads.view.help"));
            items.put("search", CustomHeads.getTagEditor().setTags(new ItemEditor(Material.NAME_TAG).setDisplayName(CustomHeads.getLanguageManager().ITEMS_SEARCH).setLore(CustomHeads.getLanguageManager().ITEMS_SEARCHLORE).getItem(), "chItem", "search", "needsPermission", "heads.use.more.search"));
            items.put("find_more", CustomHeads.getTagEditor().setTags(new ItemEditor(Material.PAPER).setDisplayName(CustomHeads.getLanguageManager().ITEMS_FIND_MORE).setLore(CustomHeads.getLanguageManager().ITEMS_FIND_MORELORE).getItem(), "chItem", "find_more"));
            items.put("saved_heads", CustomHeads.getTagEditor().setTags(new CustomHead(getURLFrom("eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvNzM3NjQ4YWU3YTU2NGE1Mjg3NzkyYjA1ZmFjNzljNmI2YmQ0N2Y2MTZhNTU5Y2U4YjU0M2U2OTQ3MjM1YmNlIn19fQ==")).setDisplayName(CustomHeads.getLanguageManager().ITEMS_COLLECTION).toItem(), "chItem", "saved_heads"));
            ...
            for (JsonElement menuElement : menus) {
                ...
                for (JsonElement contentElement : menuJson.getAsJsonArray("content")) {
                    ...
                    switch (contentObject.get("type").getAsString()) {
                        case "category":
                            Category category = CustomHeads.getCategoryImporter().getCategories().get(contentObject.get("id").getAsInt() + "");
                            if (category == null)
            ...
            for (Map.Entry<String, JsonElement> slotEntry : slots.entrySet()) {
                ...
                Category category = CustomHeads.getCategoryImporter().getCategories().get(slotEntry.getKey());
                if (category != null && category.hasSubCategories()) {
                    for (JsonElement contents : slotEntry.getValue().getAsJsonObject().get("contents").getAsJsonArray()) {
                        ...
                        if (subcategoryObject.get("type").getAsString().equals("category")) {
                            SubCategory subCategory = CustomHeads.getCategoryImporter().getSubCategory(category.getId() + ":" + subcategoryObject.get("id"));
                            if (subCategory == null)
    ...
    public HashMap<String, Inventory> getCreatedMenus() {
        return createdMenus;
    }
    ...
    public HashMap<String, String[]> getAllMenuInfos() {
        return menuInfo;
    }
